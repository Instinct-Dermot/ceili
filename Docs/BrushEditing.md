...
<img width="880" height="534" alt="The Studio Brushes analytics panel: 1900 brush surfaces merged into 205 draws (9.3x fewer), with the per-material batch table and the batch-bounds overlay on the 2D view" src="Media/brush_batches.webp" />

*The merged-batch panel on a 1900-brush level import: 1900 per-brush draws collapse
to 205 batches, 9.3x fewer, keyed by spatial leaf and material. The 2D overlay
draws each batch's bounds, the per-material table breaks the batches down (note the
`studio/caulk` and `studio/nodraw` tool surfaces), and the toggles stand the
individual brushes down in favour of the merged geometry.*
